...
| 3 | `exact_version_agreement`, `validate_model_descriptor`, `dependency_reachability`, `_record_for_dependency`, `_dependency_associations`, `validate_packages`, resolved-ContractSpec dispatch | core model, descriptor, service, fragments, package, ABI, typed target, literal authoritative `E_b/E_c/E_lex`, typed dependency scopes, and exact `TR/TRB/ROOT_TR_c` records | `test_k3x_03_exact_versions_and_complete_descriptor_validation`; mutates requested/model version, ABI, plugin, judgment, target, fragments, typed dependencies, summary and relation; independently deletes each environment/root, supplies a foreign trust root, and attempts a `RecordIdentity` summary scope | K2 §§3.3–3.4, 4.3, 8.2; K3-S §§4.3, 6, 9.6 check 3 | every descriptor root resolves by exact `recordAt` or a unique typed association; no name-table carrier, synthetic record, trust-root exception, or untyped model-summary scope remains; missing, ambiguous, foreign, or wrong-kind roots reject |
...
| 5 | `_capability_target_roots`, `_bindings_for_declaration`, `_syntax_associations`, `_proper_graph`, `topological_order`, `_producer_set`, `validate_pair`, `_validate_retained_ck_exact` | exact pair declaration, `PB_alt`, occurrence bundle, occurrence model, proof/certificate/validator records, and retained cycle reconstruction | `test_k3x_05_pair_producers_validation_refs_and_proper_cycle`; deletes `PB_alt`, independently derives the literal `SP(refresh_occurred)` association/environment, asserts the three exact `PairTarget` roots, retains all pair falsifiers, and proves the genuine proper cycle | K2 §§3.3–3.4, 5.1–5.2, 6.4–6.5, 7.2; K3-S §§3, 9.2, 9.5–9.6 check 5 | a pair target contains its declaration, matching pair binding, and occurrence bundle; ordinary and pair-owned occurrence associations are total and unique; missing/ambiguous pair ownership rejects; validation references remain outside the proper DAG |
...
- Dependency/pair: checks 3, 5, and 9 cover authoritative carrier/root deletion,
  foreign roots, typed model-summary scopes, missing/wrong binding identities, occurrence-aware associations, query-derived
