...
|---:|---|---|---|---|---|
| 1 | `compose_records`, `validate_packages`, `evaluate_invocation`, `observe`, `verification_passed`, `task_accepts` | `core_construction` with literal `PKG_CK`, embedded Delta admission contracts, `E_t`, `D_t`, `T_admitted`, `Q_t`, and `RES_t` records | `test_k3x_01_core_definitional_and_closed_coding_values`; removes and empties the package, reduces a type dependency projection, changes the embedded authoritative admission domain, checks absent, role-mismatch, projection-mismatch, PASS/FAIL conflict, OTHER role/format, FieldValue tags, extensional snapshot maps, and the value-independent four-equation verification counterexample | K2 §§2.4, 3.3, 4.1–4.3, 5.1–5.2; K3-S §§2.1–2.2, 3, 4, 9.1, 9.6 check 1 | exact `TRUE`; missing or producer-reduced authoritative construction is malformed; opposite decisive evidence is evaluation error |
...
| 5 | `_proper_graph`, `topological_order`, `_producer_set`, `validate_pair` | `pair_construction` with exact occurrence/lift roles, typed pair target/use/trust/admission/result carriers, both models, literal `PAIR/PB_alt`, `E_p/D_p`, four Service specs, `PVC`, `R_p`, evidence, certificate, admitted `TRP`, and validation refs; `cycle_universe` with the isolated 24-type Delta/model/package closure | `test_k3x_05_pair_producers_validation_refs_and_proper_cycle`; removes either model and a cycle model, mutates every `D_p` set, removes producer/reference, aliases producer owners, changes certificate subject, and empties the pair package | K2 §§3.3, 7.2; K3-S §§3, 9.2, 9.5–9.6 check 5 | exact typed `PairValidationResult`; every pair coordinate is consumed; missing/incompatible trust or self/equal producer sets fail; only the complete non-cycle-valid construction reaches `MALFORMED(dependency cycle)` |
...
| 9 | `validate_contract_spec`, `evaluate_observation_graph` | `confluence_construction` with independent `N_o/N_d` ContractSpecs and literal `D_c/R_c/T_c/E_c/M_c/Y_c/L_c_final/K_c` links; four permutation universes with Delta admissions | `test_k3x_09_confluence_contractspec_graph_errors_and_permutations`; absent/wrong-kind node, support mismatch, duplicate query, foreign/empty mutations of all seven dependency-environment coordinates | K2 §§1.1, 3.3, 7.5, 8.3; K3-S §§4.2, 9.5–9.6 check 9 | both orders yield identical complete maps/status; every dependency coordinate and downstream carrier link is authoritative; four full presentations compose identically |
...
| 11 | replay `LookupRequest`, `ResolutionCoordinate`, `_resolve_failed_coordinate`; typed migration/compatibility/extension/alias validation | the explicit `LITERAL_MISSING_ROWS` tuple: 42 canonical targets, complete named baseline/variant containers, finite typed evolution relations/certificates/validators, literal manifests, and explicit removals/additions | `test_k3x_11_all_42_literal_missing_reconstructions`; asserts all 42 complete target identities, manifest equality, 1→0 target count, exact symmetric differences, empty records/packages, extraneous lexical reconstruction, arbitrary coordinates, and a same-local foreign consumer identity | K2 §3.3 `recordAt`/`missingStatus`, §2.4; K3-S §§8, 9.4 and check 11 | status derives only from a frozen complete identity coordinate and validated consumer relation; arbitrary/foreign coordinates raise `FiniteProfileError`; evolution rows traverse typed validation paths |
