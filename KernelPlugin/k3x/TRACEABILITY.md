...
|---:|---|---|---|---|---|
| 1 | `compose_records`, `validate_packages`, `evaluate_invocation`, `observe`, `verification_passed`, `task_accepts` | `core_construction` with `PKG_CK`, `E_t`, `D_t`, `T_admitted`, and identity-only `Q_t` | `test_k3x_01_core_definitional_and_closed_coding_values`; removes and empties the package, checks absent, role-mismatch, projection-mismatch, and PASS/FAIL conflict | K2 §§2.4, 3.3, 4.1–4.3, 5.1–5.2; K3-S §§2.1–2.2, 4, 9.1, 9.6 check 1 | exact `TRUE`; missing authoritative construction is malformed; opposite decisive evidence is evaluation error |
...
| 3 | `exact_version_agreement`, `validate_model_descriptor`, `validate_packages` | core model, descriptor, service, fragments, package, ABI, target, trust-root identities | `test_k3x_03_exact_versions_and_complete_descriptor_validation`; mutates requested/model version, ABI, plugin, judgment, target, fragment role, dependency closure, validation/proper separation, trust-root kind, and summary | K2 §§3.4, 4.3, 8.2; K3-S §§4.3, 6, 9.6 check 3 | exact version only; every mutated projection is malformed or version mismatch; no fallback |
...
| 5 | `_proper_graph`, `topological_order`, `_producer_set`, `validate_pair` | `pair_construction` with pair declaration/binding, certificate, exactly two non-proper validation refs, trust chain, and producer records; `cycle_universe` | `test_k3x_05_pair_producers_validation_refs_and_proper_cycle`; removes producer/reference, aliases producer owners, changes certificate subject, empties pair package | K2 §§3.3, 7.2; K3-S §§9.2, 9.5–9.6 check 5 | exact `PairValidationResult`; missing producer/reference, self/equal producer sets, wrong subject fail; mechanically derived proper back-edge is `MALFORMED(dependency cycle)` |
...
| 9 | `validate_contract_spec`, `evaluate_observation_graph` | `confluence_construction` with literal `N_o/N_d` ContractSpecs and their exact observation-query map; four presentation universes | `test_k3x_09_confluence_contractspec_graph_errors_and_permutations`; absent/wrong-kind node, support mismatch, duplicate query | K2 §§1.1, 3.3, 7.5, 8.3; K3-S §§4.2, 9.5–9.6 check 9 | both orders yield identical complete maps; graph/schema defects fail; four presentations compose identically |
| 10 | `compose_records` conflict construction | `duplicate_universe` forward/reverse equal and unequal complete declarations | `test_k3x_10_duplicate_equal_and_conflict_are_order_independent` | K2 §§1.1, 8.3; K3-S §9.5 and check 10 | equal duplicate coalesces; unequal duplicate produces the same two-record conflict in either order |
| 11 | replay `LookupRequest`, structural context-root admission, `_missing_result` | 42 `missing_construction` calls with exact §9.4 row tags/names, complete target records, retained row anchors, rebuilt packages/environments/requests/lifecycle/conflict carriers | `test_k3x_11_all_42_literal_missing_reconstructions`; for every row checks 1→0 target count, empty universe, emptied nested package; separately checks real extraneous lexical environment/request | K2 §3.3 `recordAt`/`missingStatus`, §2.4; K3-S §9.4 and check 11 | exact displayed family only after the literal row context resolves; empty records/packages are `MALFORMED_CONTEXT`, never an answer |
| 12 | public `replay(Universe)` | `_build_packet`: 1 core + 1 pair + 5 trust + 42 bases + 42 variants + 2 confluence + 1 cycle + 4 permutations + 2 equal + 2 conflict | `test_k3x_12_all_102_replays_without_identifier_or_assertion_input`; independently replays every universe and compares only afterward | K1 §4.8; K2 §§1.1, 3.3, 5.2; K3-S §§9.1, 9.6 check 12 | domains equal and cardinality 102; 42/42 rows; evaluator receives no tag or assertion value |
...
- Dependency/pair: checks 5 and 9 cover missing/wrong graph nodes, query-derived
...
- Missing/lifecycle: check 11 iterates all 42 closed tags. Optional versus
  required extension/alias behavior comes from the reconstructed evolution
  requirement record; Sigma versus Service ContractSpec behavior comes from
