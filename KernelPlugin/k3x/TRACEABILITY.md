...
|---:|---|---|---|---|---|
| 1 | `compose_records`, `validate_packages`, `evaluate_invocation`, `observe`, `verification_passed`, `task_accepts` | `core_construction` with literal `PKG_CK`, `E_t`, `D_t`, `T_admitted`, `Q_t`, and `RES_t` records | `test_k3x_01_core_definitional_and_closed_coding_values`; removes and empties the package, checks absent, role-mismatch, projection-mismatch, PASS/FAIL conflict, OTHER role/format, FieldValue tags, extensional snapshot maps, and the value-independent four-equation verification counterexample | K2 §§2.4, 3.3, 4.1–4.3, 5.1–5.2; K3-S §§2.1–2.2, 4, 9.1, 9.6 check 1 | exact `TRUE`; missing authoritative construction is malformed; opposite decisive evidence is evaluation error |
...
| 5 | `_proper_graph`, `topological_order`, `_producer_set`, `validate_pair` | `pair_construction` with literal `B_alt/PB_alt`, `E_p/D_p`, four Service specs, `PVC`, `R_p`, `PENV`, evidence, certificate, admitted `TRP`, and certificate/capability validation refs; `cycle_universe` query maps | `test_k3x_05_pair_producers_validation_refs_and_proper_cycle`; removes producer/reference, aliases producer owners, changes certificate subject, empties pair package | K2 §§3.3, 7.2; K3-S §§9.2, 9.5–9.6 check 5 | exact `PairValidationResult`; one certificate and one capability ref are non-proper; missing/incompatible trust or self/equal producer sets fail; query-derived proper back-edge is `MALFORMED(dependency cycle)` |
