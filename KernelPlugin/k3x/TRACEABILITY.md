...
|---:|---|---|---|---|---|
| 1 | `admit_typed_value`, `admitted_closed_value`, `compose_records`, `validate_packages`, `evaluate_invocation`, `observe`, `verification_passed`, `task_accepts` | `core_construction` with literal `PKG_CK`, field-sensitive Delta `TypeAdmissionRelation`s, `E_t`, `D_t`, `T_admitted`, `Q_t`, and `RES_t` | `test_k3x_01_core_definitional_and_closed_coding_values`; forges nested `TaskSpec(object)`, non-`Path` path-set members, subject tags, correspondence payloads, empty conflicts and bool-as-int `ByteSize`; admits explicit other-role/format atoms and distinguishes command/contact/release/snapshot wrappers | K2 §§2.4, 3.2–3.3, 4.1–4.3, 5.1–5.2; K3-S §§2.1–2.2, 3.2, 4.1, 9.1, 9.6 check 1 | exact `TRUE`; every retained constructor and nested field has an explicit exact membership predicate; malformed nested/wrong-constructor values reject without a tag oracle |
...
| 5 | `_proper_graph`, `topological_order`, `_producer_set`, `validate_pair` | exact seven-field `PairAdmissionRequest`, independent `PairBinding` admission, least `types_p` closure, three typed literal declarations/bindings/models, full `PAIR/PB_alt`, `E_p/D_p`, four Service specs, `PVK/PVSK/PVC`, envelope/evidence, admitted `TRP`, and separate support producers; retained service/certificate-free acyclic/cyclic package variants | `test_k3x_05_pair_producers_validation_refs_and_proper_cycle`; corrupts each literal family, exact evidence coordinates, admission, request/binding/model fields and each certificate/validation/trust producer; proves the retained acyclic baseline and exact four-identity cyclic differential | K2 §§3.3, 5.1–5.2, 6.4–6.5, 7.2; K3-S §§3, 9.2, 9.5–9.6 check 5 | exact separately resolved typed pair result; full type/literal closure and producer independence; all retained cycle surroundings are equal and only the isolated back-edge yields `MALFORMED(dependency cycle)` |
...
| 9 | `validate_contract_spec`, `validate_binding`, `validate_model_descriptor`, `evaluate_observation_graph` | complete retained `PKG_CK` plus exact confluence declarations/bindings/models, typed `C_c`, `U_c=(IDENTITY_OF(C_c),IDENTITY_OF(M_c))`, and exact `D_c/R_c/T_c/E_c/M_c/Y_c/L_c_final/K_c` links | `test_k3x_09_confluence_contractspec_graph_errors_and_permutations`; corrupts facets, removes each declaration/binding/model, and mutates policy/root/service/descriptor/producers plus every graph coordinate | K2 §§1.1, 2.3, 3.3, 6.1–6.5, 7.5, 8.3; K3-S §§4.2, 6.2–6.3, 9.5–9.6 check 9 | declarations are individually well formed and bindings closed before graph evaluation; both orders yield identical complete maps/result/status; exact typed identity and trust links are authoritative |
...
| 11 | replay `LookupRequest`, `ResolutionCoordinate`, `_resolve_failed_coordinate`; typed migration/compatibility/extension/alias validation | 42 literal rows plus distinct migration/compatibility/semantic-extension request/result constructors, row-specific failure codomains, exact envelopes/evidence and policy/root/adoption links | `test_k3x_11_all_42_literal_missing_reconstructions`; retains all identity/manifest checks and corrupts envelope fragment/abstraction, all evidence coordinates, policy owner, request/result sort, and removes every row support producer | K2 §§2.3, 3.3, 6.4–6.5, 8.2; K3-S §§8, 9.4 and check 11 | no generic evolution carrier or row-string dispatch; exact typed request/result, evidence, policy/root link, producer, certificate admission and receiving result are required |
