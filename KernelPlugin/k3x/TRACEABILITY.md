...
|---:|---|---|---|---|---|
| 1 | `admit_typed_value`, `admitted_closed_value`, `compose_records`, `validate_packages`, `_validate_retained_ck_exact`, `evaluate_invocation`, `observe`, `verification_passed`, `task_accepts` | one literal `PKG_CK`: exact 55 `DELTA_TYPE_X`, 47 `DELTA_LITERAL_X`, 17 `DELTA_SYMBOL_X`, ordinary/pair/profile Sigma records, all model contracts, six field-complete capabilities, exact `BENV/BCERT`, and field-sensitive admission relations; shared by core, confluence, and cycle fixtures | `test_k3x_01_core_definitional_and_closed_coding_values`; asserts every admission domain/codomain/relation/support field, the exact `SELECT_PATHS` and `NO_SYMBOL_LITERAL` rows, callable ContractSpecs and complete descriptor values, six authority attestations/evidence references, exact envelope/key fields, and independently mutates every descriptor and retained callable-contract field | K2 §§2.4, 3.2–3.3, 4.1–4.3, 5.1–5.2; K3-S §§2.1–2.2, 3.2, 4.1, 6.2, 8.2, 9.1, 9.6 check 1 | exact `TRUE`; every retained type, callable, profile projection, descriptor, and envelope field is compared with frozen values; candidate-derived equality cannot preserve the accepted assertion |
...
| 9 | `validate_contract_spec`, `validate_binding`, `validate_model_descriptor`, `evaluate_observation_graph` | shared retained package; `E_c` contains exact retained declarations/bindings/models for `observe`, `changes_between`, `observations_equal`, and `dependency_metadata_changed`, four frozen literals, exact authority records, and exact mechanically extracted roots; `D_c` contains independently recomputed syntax/subject/association/expanded/proper/transitive fields | `test_k3x_09_confluence_contractspec_graph_errors_and_permutations`; proves all four retained lower identities and models, mutates every clause/adoption/source/authority/fact/environment/dependency/service/trust/producer coordinate, and rejects missing, extra, or coherently rewritten environment members | K2 §§1.1, 2.3, 3.3, 6.1–6.5, 7.5, 8.3; K3-S §§4.2, 6.2–6.3, 9.5–9.6 check 9 | both orders yield the identical complete exact map/result/status; only the frozen full `E_c/D_c` domain is accepted, with two independent observed lower nodes |
...
| 11 | replay `LookupRequest`, `ResolutionCoordinate`, `_resolve_failed_coordinate`; `_frozen_task_environment_value`, `_frozen_task_values`; typed migration/compatibility/extension/alias validation | 42 literal rows plus distinct evolution request/result constructors; exact `Delta_t` is the least nested type closure plus `t_t/F_t`, exact `DP(task_accepts)` declaration/binding/model/ContractSpecs, exact `E_t`, independently frozen `required(S_t)` and `D_t`, and four complete evolution admission chains | `test_k3x_11_all_42_literal_missing_reconstructions`; corrupts source/target/request identities and complete values, injects coherent extra members, rogue `chi_c`, mechanical/authority mutations, wrong member kinds, dangling dependencies, and every envelope/evidence/policy/request/result/producer coordinate | K2 §§2.3, 3.3, 6.4–6.5, 8.2; K3-S §§8, 9.4 and check 11 | source and target roots equal the exact frozen `E_t`; exact member domains, empty `chi_c`/authority/choice/profile fields, complete `D_t`, request arguments, trust, certificate admission, and receiving result are mandatory |
