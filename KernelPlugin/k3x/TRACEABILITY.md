...
|---:|---|---|---|---|---|
| 1 | `admit_typed_value`, `admitted_closed_value`, `compose_records`, `validate_packages`, `evaluate_invocation`, `observe`, `verification_passed`, `task_accepts` | `core_construction` with literal `PKG_CK`, field-sensitive Delta `TypeAdmissionRelation`s, `E_t`, `D_t`, `T_admitted`, `Q_t`, and `RES_t` | `test_k3x_01_core_definitional_and_closed_coding_values`; forges nested `TaskSpec(object)`, forges bool-as-int `ByteSize`, changes the authoritative relation, and retains the prior observation/evidence/extensional falsifiers | K2 §§2.4, 3.2–3.3, 4.1–4.3, 5.1–5.2; K3-S §§2.1–2.2, 3.2, 4.1, 9.1, 9.6 check 1 | exact `TRUE`; only exact constructor and recursively admitted fields are admitted; malformed nested/wrong-constructor values reject without a tag oracle |
...
| 5 | `_proper_graph`, `topological_order`, `_producer_set`, `validate_pair` | exact seven-field `PairAdmissionRequest`, distinct occurrence-binding/B_alt identities, standalone `CertificateAdmission` and `PairValidationResult`, full `PAIR/PB_alt`, `E_p/D_p`, four Service specs, `PVK/PVSK/PVC`, both models, envelope/evidence, admitted `TRP`, and validation refs; complete service-free acyclic/cyclic package variants | `test_k3x_05_pair_producers_validation_refs_and_proper_cycle`; mutates every request, pair-binding, scope-binding, and model field, the bundle, every standalone admission/result field and identity, dependencies/producers/trust; proves the acyclic baseline first and that the sole frozen back-edge yields the cycle | K2 §§3.3, 5.1–5.2, 6.4–6.5, 7.2; K3-S §§3, 9.2, 9.5–9.6 check 5 | exact separately resolved typed pair result; no request-embedded answer; full model/signature/scope/trust equality; only the isolated back-edge yields `MALFORMED(dependency cycle)` |
...
| 9 | `validate_contract_spec`, `evaluate_observation_graph` | `confluence_construction` keyed directly by `BINDING(DF(observe))`/`BINDING(DF(changes_between))`, typed `ConfluenceSubject`, `ReasoningTarget`, `EnvironmentUse`, `ReasoningUnknown/ResultValue`, and exact `D_c/R_c/T_c/E_c/M_c/Y_c/L_c_final/K_c` links | `test_k3x_09_confluence_contractspec_graph_errors_and_permutations`; mutates inputs, service, root owner/validators/kinds/targets/adoption, result reason, producer, queries, and all seven dependency coordinates | K2 §§1.1, 2.3, 3.3, 6.1–6.5, 7.5, 8.3; K3-S §§4.2, 6.2–6.3, 9.5–9.6 check 9 | both orders yield identical complete maps/result/status; no observation-node surrogate; exact typed `U_c`, service, descriptor, producer, root, result, lifecycle and status are authoritative |
...
| 11 | replay `LookupRequest`, `ResolutionCoordinate`, `_resolve_failed_coordinate`; typed migration/compatibility/extension/alias validation | 42 literal rows plus typed `MigrationAdmissionSubject`, `CompatibilityAdmissionSubject`, `SemanticExtensionAdmissionSubject`, typed proof/trust targets/conclusions, four full services/fragments/envelopes/admissions/results and producer sets | `test_k3x_11_all_42_literal_missing_reconstructions`; retains all identity/manifest/foreign-coordinate checks and corrupts evolution root, service, proof, capability scope/root, fragment relation, admission and receiving result | K2 §§2.3, 3.3, 6.4–6.5, 8.2; K3-S §§8, 9.4 and check 11 | no row-string/local-name dispatch; exact subject, environment, service, evidence, scope, root, producer, certificate admission and receiving result are required |
