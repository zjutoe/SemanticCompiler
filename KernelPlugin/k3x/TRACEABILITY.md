...
expansions that produce 102 assertion-only projections. Each projection
freezes the complete canonical authoritative record map as a lossless
immutable value graph, plus the conflict coordinate and semantic outcome,
without reading a constructed universe, manifest, request, evidence value,
result value, or replay.
...
| 3 | `exact_version_agreement`, `validate_model_descriptor`, `dependency_reachability`, `_record_for_dependency`, `_dependency_associations`, `_validate_retained_authoritative_values`, `validate_packages`, resolved-ContractSpec dispatch | core model, descriptor, service, fragments, package, ABI, typed target, independently specified complete authoritative `E_b/E_c/E_lex`, typed dependency scopes, and complete exact `TR/TRB/ROOT_TR_c` records | `test_k3x_03_exact_versions_and_complete_descriptor_validation`; mutates requested/model version, ABI, plugin, judgment, target, fragments, typed dependencies, summary and relation; independently deletes, empties, and coherently alters each environment, and mutates every trust-root field including a foreign-scoped same-identity root | K2 §§3.3–3.4, 4.3, 8.2; K3-S §§4.3, 6, 9.6 check 3 | every descriptor root resolves to its independently frozen complete value or a unique typed association; missing, ambiguous, foreign-scoped, wrong-kind, or same-identity/wrong-value roots reject |
...
| 5 | `_capability_target_roots`, `_record_for_dependency`, `_bindings_for_declaration`, `_syntax_associations`, `_proper_graph`, `topological_order`, `_producer_set`, `validate_pair`, `_validate_retained_ck_exact` | exact pair declaration, declaration-derived occurrence binding key, `PB_alt`, occurrence bundle/model, proof/certificate/validator records, and retained cycle reconstruction | `test_k3x_05_pair_producers_validation_refs_and_proper_cycle`; the independent oracle requires exact occurrence-binding ownership; foreign, missing, and ambiguous owners reject; the three exact `PairTarget` roots, all pair falsifiers, and genuine proper cycle remain covered | K2 §§3.3–3.4, 5.1–5.2, 6.4–6.5, 7.2; K3-S §§3, 9.2, 9.5–9.6 check 5 | pair-owned association requires pair key, occurrence symbol, bundle, and `PairBinding.occurrence_binding_key=RecordIdentity(BINDING, occurrence_declaration.key)`; validation references remain outside the proper DAG |
| 6 | `_trust`, `evaluate_invocation`, retained descriptor validation | five separately constructed `Q_t[T_x]` universes with policy/root/environment records | `test_k3x_06_five_trust_lifecycle_and_discovery_branches`; removes the trust-environment record and separately removes the authoritative root record | K2 §§2.3–2.4, 5.4, 8.1; K3-S §§9.3, 9.6 check 6 | admitted/available; judgment-level absent/missing; undecided/unknown; incompatible/missing; discovery failure/unknown; deleting the root record itself makes the retained composition malformed rather than fabricating `ABSENT` |
...
| 12 | public `replay(Universe)` followed only by an assertion-side complete-map projection | `_build_fixture_packet`: 1 core + 1 pair + 5 trust + 42 bases + 42 variants + 2 confluence + 1 cycle + 4 permutations + 2 equal + 2 conflict; explicit immutable node and root literals freeze all 102 complete authoritative maps independently of construction | `test_k3x_12_all_102_replays_without_identifier_or_assertion_input`; independently replays every universe; validates every node hash and reference; proves distinct nodes for the exact 56 ordinary and nine enum admission classes; rejects an unsupported object; and proves six corruptions diverge, including an outcome/conflict-preserving coherent `PathSegment`→`Path` admission-class substitution | K1 §4.8; K2 §§1.1, 3.3, 5.2; K3-S §§9.1, 9.6 check 12 | domains equal and cardinality 102; 42/42 rows; exact `RecordIdentity -> complete immutable value`, conflict, and outcome coordinates equal; represented classes use module/qualname tags, and unsupported values fail loudly; evaluator receives no tag or assertion value |
...
- Assertion independence: check 12 compares only after public replay against a
  ...
  from the assertion builder. The reachable immutable value kinds are `None`,
  integer, string, enum value, ordinary or enum class, dataclass, tuple, and
  frozenset; each has an unambiguous structural tag, while other objects raise.
  Hash/reference checks, class-node cardinalities, and six independent
  mutations demonstrate that the table does not move with fixture or result
  changes.
...
- Dependency/pair: checks 3, 5, and 9 cover authoritative carrier/root deletion,
  foreign and same-identity/wrong-value roots, typed model-summary scopes,
  missing/wrong binding identities, exact occurrence-owned associations, query-derived
