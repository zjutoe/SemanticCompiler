...
|---:|---|---|---|---|---|
| 1 | `admit_typed_value`, `admitted_closed_value`, `compose_records`, `validate_packages`, `evaluate_invocation`, `observe`, `verification_passed`, `task_accepts` | one literal `PKG_CK`: exact 55 `DELTA_TYPE_X`, 47 `DELTA_LITERAL_X`, 17 `DELTA_SYMBOL_X`, ordinary/pair/profile Sigma records, all model contracts, six field-complete capabilities, exact `BENV/BCERT`, and field-sensitive admission relations; shared by core, confluence, and cycle fixtures | `test_k3x_01_core_definitional_and_closed_coding_values`; asserts every admission domain/codomain/relation/support field, every literal declaration/binding/model bijection, six authority attestations and evidence references, every descriptor coordinate/target/root/scope, exact envelope fields, positive witnesses, and malformed nested counterexamples | K2 §§2.4, 3.2–3.3, 4.1–4.3, 5.1–5.2; K3-S §§2.1–2.2, 3.2, 4.1, 6.2, 8.2, 9.1, 9.6 check 1 | exact `TRUE`; each retained type row has primary domain `Value`, codomain `{admitted,not_admitted}`, recursive support, and exact membership relation; retained literals and descriptors are checked field-by-field rather than by cardinality alone |
...
| 5 | `_proper_graph`, `topological_order`, `_producer_set`, `validate_pair` | accepted pair packet unchanged; cycle replaces only retained `event_matches` binding/meaning/model projection while the retained `event_occurred` binding remains value-identical and resolves its support through that same-key replacement | `test_k3x_05_pair_producers_validation_refs_and_proper_cycle`; preserves all pair falsifiers, proves baseline acyclicity, asserts the exact cycle changed set, byte/value equality of `event_occurred`, and equality of every unrelated retained record | K2 §§3.3, 5.1–5.2, 6.4–6.5, 7.2; K3-S §§3, 9.2, 9.5–9.6 check 5 | accepted pair result is preserved; replacement reachability yields `MALFORMED(dependency cycle)` before a stale derived-closure check can require rewriting the unchanged predecessor |
...
| 9 | `validate_contract_spec`, `validate_binding`, `validate_model_descriptor`, `evaluate_observation_graph` | shared retained package; `N_o/N_d` are exactly retained `coding.binding` members for `observe/changes_between`; `E_c/D_c` include their retained declarations/bindings, the four exact retained literals, authority facts, and recomputed support closure; no `confluence.binding/declaration/model` semantic namespace remains | `test_k3x_09_confluence_contractspec_graph_errors_and_permutations`; proves package equality and retained node membership, removes a retained authority literal and an `E_c` literal binding, corrupts every clause/adoption/authority/environment/dependency/service/trust/producer coordinate, and deletes each retained node model through its package | K2 §§1.1, 2.3, 3.3, 6.1–6.5, 7.5, 8.3; K3-S §§4.2, 6.2–6.3, 9.5–9.6 check 9 | both orders yield the identical complete exact map/result/status; package membership and every environment field are validated, and deletion of retained members cannot leave a parallel graph result |
...
| 11 | replay `LookupRequest`, `ResolutionCoordinate`, `_resolve_failed_coordinate`; typed migration/compatibility/extension/alias validation | 42 literal rows plus distinct evolution request/result constructors, one exact source/target/request `SemanticEnvironment`, environment-derived roots/closure, typed dependency environments, envelopes/evidence and policy/root/adoption links | `test_k3x_11_all_42_literal_missing_reconstructions`; corrupts source/target/request identities and contents, explicitly injects rogue `chi_c`, `MK0` as mechanical dependency, `MK0` as authority fact, dangling dependencies, and every envelope/evidence/policy/request/result/producer coordinate | K2 §§2.3, 3.3, 6.4–6.5, 8.2; K3-S §§8, 9.4 and check 11 | exact source/target equality, member kinds, empty `chi_c`, exact mechanical dependency set, empty authority/choice/profile fields, derived closure, evidence, trust, certificate admission and receiving result are all required |
