...
| 12 | public `replay(Universe)` followed only by an assertion-side complete-map projection | `_build_fixture_packet`: 1 core + 1 pair + 5 trust + 42 bases + 42 variants + 2 confluence + 1 cycle + 4 permutations + 2 equal + 2 conflict; explicit immutable node and root literals freeze all 102 complete authoritative maps independently of construction | `test_k3x_12_all_102_replays_without_identifier_or_assertion_input`; independently replays every universe and proves exactly five corruptions diverge: extra record, changed evidence, changed `ResultRecord`, empty `E_b`, and foreign-scoped same-identity `TRB` | K1 §4.8; K2 §§1.1, 3.3, 5.2; K3-S §§9.1, 9.6 check 12 | domains equal and cardinality 102; 42/42 rows; exact `RecordIdentity -> complete immutable value`, conflict, and outcome coordinates equal; evaluator receives no tag or assertion value |
...
- Assertion independence: check 12 compares only after public replay against a
  ...
  from the assertion builder; five independent mutations demonstrate that
