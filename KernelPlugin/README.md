...
> Status: K0 is accepted and complete; the K1 handoff is a review candidate and grants no execution authority.
...
K0 freezes the design inputs for K1; it does not select the kernel calculus, plugin ABI, coding symbols, evaluator implementation, or execution architecture. The [K1 handoff candidate](handoffs/K1_kernel_calculus_and_semantics.md) defines the next semantic-design boundary, but it is not accepted or dispatched.
...
| K1 | Kernel calculus and denotational semantics | Handoff candidate; review required |
